...
namespace NetDimension.Weibo
{
	...
	public class OAuth
	{
        ...
        internal string SearchRequest(string url, RequestMethod method = RequestMethod.Get, params WeiboParameter[] parameters)
        {
            string rawUrl = string.Empty;
            UriBuilder uri = new UriBuilder(url);
            WeiboParameter[] parameterList;
            if (url.StartsWith("https://api.weibo.com/2/search/topics.json"))
            {
                parameterList = new WeiboParameter[parameters.Length + 2];
                ...
                if (!string.IsNullOrEmpty(AccessToken))
                {
                    parameterList[parameters.Length] = new WeiboParameter("access_token", AccessToken);
                    parameterList[parameters.Length + 1] = new WeiboParameter("source", AppKey);
            ...
            {
                parameterList = new WeiboParameter[parameters.Length];
                for (int i = 0; i < parameters.Length; i++)
                {
                    parameterList[i] = parameters[i];
                }
            ...
            bool multi = false;
#if !NET20
            multi = parameterList.Count(p => p.IsBinaryData) > 0;
#else
			foreach (var item in parameters)
			{
				if (item.IsBinaryData)
            ...
            switch (method)
            {
                case RequestMethod.Get:
                    {
                        uri.Query = Utility.BuildQueryString(parameterList);
                    }
                    break;
                case RequestMethod.Post:
                    {
                ...
                case RequestMethod.Post:
                    {
                        ...
                        if (multi)
                        {
                            ...
                            using (Stream request = http.GetRequestStream())
                            {
                                try
                                {
                                    var raw = Utility.BuildPostData(boundary, parameterList);
                                    request.Write(raw, 0, raw.Length);
                                }
                                finally
                                {
                        ...
                        else
                        {
                            ...
                            using (StreamWriter request = new StreamWriter(http.GetRequestStream()))
                            {
                                try
                                {
                                    request.Write(Utility.BuildQueryString(parameterList));
                                }
                                finally
                                {
                                    request.Close();
